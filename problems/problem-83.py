        ...
        for node in neighbours(current, n):  # iterating over all possible moves from current node
            if not (node in permanent):  # so long as the node has not already been made permanent
                if node in temporary:  # if the node has been visited previously
                    if temporary[node] > permanent[current] + weight(
                            node):  # update temporary path only if new path is smaller
                        temporary[node] = permanent[current] + weight(node)
                else:
                    temporary[node] = permanent[current] + weight(
                        node)  # if the node has not been visited previously, create temporary path

        min_weight = min(temporary.values())  # minimum path over all temporary nodes
        min_nodes = [node for node in temporary if
                     temporary[node] == min_weight]  # all nodes with the minimum path - there may be more than one
...
if __name__ == '__main__':
    start = time.time()
    matrix = [[int(v) for v in line.strip().split(',')] for line in open('problem-83.txt', 'r')]
